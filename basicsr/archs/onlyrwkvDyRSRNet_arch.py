...
class SparseAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, bias, tlc_flag=True, tlc_kernel=48, activation='relu', input_resolution=None):
        ...
        self.qkv_dwconv = BSConvU(dim * 3, dim * 3, kernel_size=3, stride=1, bias=bias)
...
class RCSSblock(nn.Module):
    def __init__(
        ...
        self.self_attention = DSSM(d_model=hidden_dim, d_state=d_state, expand=expand, dropout=attn_drop_rate, **kwargs)
    ...
    def forward(self, input, x_size):
        ...
        wa_out = self.wa(self.ln_2(x).permute(0, 3, 1, 2).contiguous())  # 调整输入形状以适应 Block
        wa_out = wa_out.permute(0, 2, 3, 1).contiguous()  # 调整输出形状回到原始形状
...
class FusionMambaIR(nn.Module):
    def __init__(self,
                 ...
                 **kwargs):
        super(FusionMambaIR, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
...
if __name__ == '__main__':
    ...
    model = onlyrwkvMambaIR( upscale=2,
  in_chans=3,
  img_size=64,
  img_range=1.,
  d_state=16,
