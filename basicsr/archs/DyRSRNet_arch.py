...
wkv_cuda = load(name="wkv", sources=["/home/code/mambair/DyRSRNet/basicsr/cuda/wkv_op.cpp", "/home/code/mambair/DyRSRNet/basicsr/cuda/wkv_cuda.cu"],
...
class Spatial_Receptive_Fusion(nn.Module):
...
class Channel_Receptive_Interaction(nn.Module):
...
class Block(nn.Module):
    def __init__(self, n_embd, n_layer, layer_id, hidden_rate=4,
        ...
        self.att = Spatial_Receptive_Fusion(n_embd, n_layer, layer_id, init_mode,
                                   key_norm=key_norm)

        self.ffn = Channel_Receptive_Interaction(n_embd, n_layer, layer_id, hidden_rate,
...
class RCSSblock(nn.Module):
    def __init__(
        ...
        self.self_attention_1 = DSSM(d_model=hidden_dim // 2, d_state=d_state, expand=expand, dropout=attn_drop_rate, **kwargs)
        self.self_attention_2 = DSSM(d_model=hidden_dim // 2, d_state=d_state, expand=expand, dropout=attn_drop_rate, **kwargs)
        ...
        self.wa = Block(n_embd=hidden_dim, n_layer=1, layer_id=0, hidden_rate=4, init_mode='fancy', key_norm=False)
    ...
    def forward(self, input, x_size):
        ...
        wa_out = self.wa(self.ln_2(x).permute(0, 3, 1, 2).contiguous())
        wa_out = wa_out.permute(0, 2, 3, 1).contiguous()

        
        x = x * self.skip_scale2 + wa_out
...
class DyRSRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (x - self.mean) * self.img_range#输入归一化
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # for image denoising
            x_first = self.conv_first(x)#浅层特征提取（Shallow Feature Extraction）
            #使用一个 3×3 的卷积提取浅层特征，通道数为 embed_dim（默认96）。
            res = self.conv_after_body(self.forward_features(x_first)) + x_first#深层特征提取（Deep Feature Extraction）
...
class DyRWSSGroup(nn.Module):
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 is_light_sr = False):
        super(DyRWSSGroup, self).__init__()
